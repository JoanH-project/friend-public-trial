...
export default function CreateCase() {
  ...
  const [avatar, setAvatar] = useState<File | null>(null); const [previewUrl, setPreviewUrl] = useState<string | null>(null); const [saving, setSaving] = useState(false); const [message, setMessage] = useState("");
  const selectAvatar = (file: File | null) => { if (previewUrl) URL.revokeObjectURL(previewUrl); setAvatar(file); setPreviewUrl(file ? URL.createObjectURL(file) : null); };
  ...
  return <main><div className="noise" /><section className="shell create-page"><header className="topline"><Link href="/">← 返回案件广场</Link><span>创建新案件</span></header><div className="forum-hero"><h1>创建新案件</h1><p>来给你的朋友安排一场公开审判。</p></div><form className="crime-form create-form" onSubmit={submit}><label>昵称<input required maxLength={24} value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：小陈" /></label><label>搞笑称号<input required maxLength={36} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="例如：消息已读消失术大师" /></label><label>头像图片（可选，2MB 以内）<input type="file" accept="image/*" onChange={(e) => selectAvatar(e.target.files?.[0] || null)} /></label>{previewUrl && <div className="avatar-preview"><Avatar src={previewUrl} alt="头像预览" /><span>头像预览</span></div>}<label>最终处罚<input maxLength={48} value={punishment} onChange={(e) => setPunishment(e.target.value)} placeholder="请大家喝奶茶" /></label><p>创建后可在案件详情继续追加罪状和修改资料。</p><button disabled={saving}>{saving ? "正在立案…" : "⚔️ 创建案件"}</button>{message && <p className="notice error">{message}</p>}</form></section></main>;
}
